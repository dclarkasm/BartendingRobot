...
const float flow_amt[4][9] = {{0  , 10   , 0  , 0   , 0    , 33 , 0    , 0     , 0        },  //screw driver
                            ...
                            {0    , 5    , 0  , 0   , 0    , 0  , 39    , 0    , 4        },  //Shirley temple was 39 on sprite
                            {0    , 1    , 2  , 2   , 2    , 23  , 0    , 0    , 7        }};  //Martini (Adios) was 23 on sprite
...
void opMode(){
  ...
  while(digitalRead(manual)){        //manual control
    ...
    else if(digitalRead(up)) zup();
    else if(digitalRead(dwn)) zdwn();
    
    else if(digitalRead(sBit1)&&      //1
            !digitalRead(sBit2)&&
            !digitalRead(sBit3)&&
            !digitalRead(sBit4)
            ) sTrig1();
    else if(!digitalRead(sBit1)&&      //2
            digitalRead(sBit2)&&
            !digitalRead(sBit3)&&
            !digitalRead(sBit4)
            ) sTrig2();
    else if(digitalRead(sBit1)&&      //3
            digitalRead(sBit2)&&
            !digitalRead(sBit3)&&
            !digitalRead(sBit4)
            ) sTrig3();
    else if(!digitalRead(sBit1)&&      //4
            !digitalRead(sBit2)&&
            digitalRead(sBit3)&&
            !digitalRead(sBit4)
            ) sTrig4();
    else if(digitalRead(sBit1)&&      //5
            !digitalRead(sBit2)&&
            digitalRead(sBit3)&&
            !digitalRead(sBit4)
            ) sTrig5();
    else if(!digitalRead(sBit1)&&      //6
            digitalRead(sBit2)&&
            digitalRead(sBit3)&&
            !digitalRead(sBit4)
            ) sTrig6();
    else if(digitalRead(sBit1)&&      //7
            digitalRead(sBit2)&&
            digitalRead(sBit3)&&
            !digitalRead(sBit4)
            ) sTrig7();
    else if(!digitalRead(sBit1)&&     //8
            !digitalRead(sBit2)&& 
            !digitalRead(sBit3)&& 
            digitalRead(sBit4)
            ) sTrig8();
  ...
  while((!digitalRead(manual)) && homeStat){ //normal opperation
    ...
    !digitalRead(CL) &&
    !digitalRead(ST) &&
    !digitalRead(MT)) inCode = 1;                          //screwDriver();
    
    else if(digitalRead(CL)&&
    !digitalRead(SC) &&
    !digitalRead(ST) &&
    !digitalRead(MT)) inCode = 2;                     //shirleyTemple();
    
    else if(digitalRead(ST)&&
    !digitalRead(CL) &&
    !digitalRead(SC) &&
    !digitalRead(MT)) inCode = 3;                     //cubaLibre();
    
    else if(digitalRead(MT)&&
    !digitalRead(CL) &&
    !digitalRead(ST) &&
    !digitalRead(SC)) inCode = 4;                     //martini();
...
//===================================================================================================== Manual Control
void forward(){
  while(digitalRead(fwd) && digitalRead(manual) && !digitalRead(x_hom_sens)) { 
    //Serial.println("3");
...
void left(){
    while(digitalRead(lft) && digitalRead(manual) && !digitalRead(y_hom_sens)) {
     // Serial.println("6");
      digitalWrite(pulY, HIGH);
      digitalWrite(dirY, LOW);
      stepCntY--;
...
void zup(){
  while(digitalRead(up) && digitalRead(manual) && !digitalRead(z_hom_sens)) { 
    //Serial.println("5");
...
void zdwn(){
  while(digitalRead(dwn) && digitalRead(manual)) {
     // Serial.println("6");
      digitalWrite(pulZ, HIGH);
      digitalWrite(dirZ, HIGH);
      stepCntZ--;
...
void sTrig1(){
  while(digitalRead(sBit1)&&      //1
        !digitalRead(sBit2)&&
        !digitalRead(sBit3)&&
        !digitalRead(sBit4) && 
        digitalRead(manual)) {
    digitalWrite(vodOut, HIGH);
    }
}
void sTrig2(){
  while(!digitalRead(sBit1)&&      //2
        digitalRead(sBit2)&&
        !digitalRead(sBit3)&&
        !digitalRead(sBit4) && 
        digitalRead(manual)) {
    digitalWrite(rumOut, HIGH);
    }
}
void sTrig3(){
  while(digitalRead(sBit1)&&      //3
        digitalRead(sBit2)&&
        !digitalRead(sBit3)&&
        !digitalRead(sBit4) && 
        digitalRead(manual)) {
    digitalWrite(ginOut, HIGH);
    }
}
void sTrig4(){
  while(!digitalRead(sBit1)&&      //4
        !digitalRead(sBit2)&&
        digitalRead(sBit3)&&
        !digitalRead(sBit4) && 
        digitalRead(manual)) {
    digitalWrite(verOut, HIGH);
    }
}
void sTrig5(){
  while(digitalRead(sBit1)&&      //5
        !digitalRead(sBit2)&&
        digitalRead(sBit3)&&
        !digitalRead(sBit4) && 
        digitalRead(manual)) {
    digitalWrite(ojOut, HIGH);
    }
}
void sTrig6(){
  while(!digitalRead(sBit1)&&      //6
        digitalRead(sBit2)&&
        digitalRead(sBit3)&&
        !digitalRead(sBit4) && 
        digitalRead(manual)) {
    digitalWrite(cokOut, HIGH);
    }
}
void sTrig7(){
  while(digitalRead(sBit1)&&      //7
        digitalRead(sBit2)&&
        digitalRead(sBit3)&&
        !digitalRead(sBit4) && 
        digitalRead(manual)) {
    digitalWrite(sprOut, HIGH);
    }
}
void sTrig8(){
  while(!digitalRead(sBit1)&& 
        !digitalRead(sBit2)&& 
        !digitalRead(sBit3)&& 
        digitalRead(sBit4) && 
        digitalRead(manual)) {
    digitalWrite(greOut, HIGH);
    }
}
...
void mix(){
  Serial.println("11");
  for(int i=0; i<=6; i++){    //nMixes=number of mixes
  //change position
  if(i==6) Zpos=0;
  else if(i==1 || i==3 || i==5) Zpos=-85000;
  else Zpos=-65000;
...
void dispense(int d_code, int posIdx){
  ...
  if(posIdx == 1) digitalWrite(78, LOW);
  else digitalWrite(69 + posIdx, LOW);
  Serial.println("flow");
  pulses = 0;
lastflowratetimer = 0;
cnt = 0;
